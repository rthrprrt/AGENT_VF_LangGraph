...
def test_parse_structured_content():
    ...
    assert "Expliquez votre fiche de poste..." in structure["Description de la mission"]
...
def test_ingest_school_guidelines_success():
    ...
    with patch(
        ...
        assert (
            updated_state.school_guidelines_formatting.get("font_name")
            == "Times New Roman"
        )
        assert "Introduction" in (updated_state.school_guidelines_structured or {})
